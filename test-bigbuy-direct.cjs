...
const API_KEY = "Nzg2NjY2Zjc0ZTJkOTQ3YWJhYmM3ZWZlZGQwZDVkNTNjMGIyMTdhNmY4NWNkMTE0MDk5YTcyYzBlNmJhYWQyNw";
...
async function testBigBuyAPI() {
  ...
  try {
    ...
    console.log(`   Rate Limit:`, authResult.headers['x-ratelimit-limit'] || 'Not provided');
  } catch (error) {
  ...
  try {
    const productsResult = await makeRequest('/rest/catalog/products.json?limit=3');
    ...
    if (productsResult.data && Array.isArray(productsResult.data)) {
      ...
      if (productsResult.data.length > 0) {
        console.log(`   First product: ${productsResult.data[0].name || 'No name'}`);
      }
    } else if (productsResult.data && productsResult.data.data) {
      console.log(`   Found ${productsResult.data.data.length} products (nested)`);
      if (productsResult.data.data.length > 0) {
        console.log(`   First product: ${productsResult.data.data[0].name || 'No name'}`);
      }
    } else {
      console.log(`   Response structure:`, Object.keys(productsResult.data || {}));
    }
  } catch (error) {
  ...
  try {
    ...
    if (categoriesResult.data && Array.isArray(categoriesResult.data)) {
      ...
      if (categoriesResult.data.length > 0) {
        console.log(`   First category: ${categoriesResult.data[0].name || 'No name'}`);
      }
    } else if (categoriesResult.data && categoriesResult.data.data) {
      console.log(`   Found ${categoriesResult.data.data.length} categories (nested)`);
      if (categoriesResult.data.data.length > 0) {
        console.log(`   First category: ${categoriesResult.data.data[0].name || 'No name'}`);
      }
    } else {
      console.log(`   Response structure:`, Object.keys(categoriesResult.data || {}));
    }
  } catch (error) {
