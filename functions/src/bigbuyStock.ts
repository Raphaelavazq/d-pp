...
// Initialize CORS with restricted origins for GDPR compliance
const corsHandler = cors.default({
  ...
  methods: ["GET"],
  credentials: false, // No credentials needed for stock checks
});
 ...
 */
export const checkBigBuyStock = functions
  .region("europe-west1") // EU region for GDPR compliance
  .runWith({
    secrets: ["BIGBUY_API_KEY"], // Secure secret handling
    enforceAppCheck: false, // Public endpoint for stock checks
  })
  .https.onRequest((request, response) => {
    corsHandler(request, response, async () => {
      try {
        ...
        const apiUrl =
          process.env.BIGBUY_API_URL || "https://api.bigbuy.eu/rest";
        ...
        const bigbuyResponse = await axios.get<BigBuyStockResponse>(
          `${apiUrl}/catalog/productStock.json`,
          {
            params: { id: productId },
            headers: {
        ...
        const stockResponse: StockApiResponse = {
          inStock,
          availableQuantity,
          productId: productId,
          timestamp: new Date().toISOString(),
