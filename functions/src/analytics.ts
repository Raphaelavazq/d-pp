...
export const generateDailyReport = functions.https.onCall(
    ...
    try {
      ...
      const ordersSnapshot = await db
        ...
        .where(
          "createdAt",
          ">=",
          admin.firestore.Timestamp.fromDate(reportDate)
        )
      ...
      const categorySales = new Map<string, CategorySalesData>();
  ...
  async (data: any, context: any) => {
    ...
    try {
      ...
      ordersSnapshot.docs.forEach((doc: any) => {
        ...
        dailyRevenue.set(
          orderDate,
          (dailyRevenue.get(orderDate) || 0) + order.total
        );
...
export const getProductAnalytics = functions.https.onCall(
  async (data: any, context: any) => {
    if (!context.auth || !context.auth.token.admin) {
      throw new functions.https.HttpsError(
        "permission-denied",
    ...
    try {
      ...
      throw new functions.https.HttpsError(
        "internal",
        "Product analytics retrieval failed"
      );
    }
  }
);
...
export const exportAnalytics = functions.https.onCall(
  async (data: any, context: any) => {
    if (!context.auth || !context.auth.token.admin) {
      throw new functions.https.HttpsError(
        "permission-denied",
    ...
    try {
      ...
      const analyticsData = analyticsSnapshot.docs.map((doc: any) =>
        doc.data()
      );
...
function convertToCSV(data: any[]): string {
  ...
  const headers = Object.keys(data[0]).filter(
    (key) => !["topProducts", "topCategories"].includes(key)
  );
...
export const analytics = {
  generateDailyReport: async () => {
    ...
    try {
      ...
      console.log(
        `Generated analytics report for ${yesterday.toISOString().split("T")[0]}`
      );
    } catch (error) {
